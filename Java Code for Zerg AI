...
public class ExampleAIClient implements BWAPIEventListener {
	...
	// counter for spawning 3 extra drones at the start
	private int startDrones;
	
	// counter for how many drones have been sent in the initial rush
	private int sentDrones;
	
	private final int WAVE1=6;
	...
	public void matchFrame() {
		...
		//get a target to focus or check if target is still alive
		for (Unit enemy : bwapi.getEnemyUnits()){
			if(target==null && enemy.getType().isBuilding()==false)
			{
				target=enemy;
				break;
			}
			...
			{
				if(enemy.getType().isBuilding()==false){
					target=enemy;
					break;
				}
			}
		...
		for (Unit unit : bwapi.getMyUnits()) {
			...
			if (unit.getType() == UnitTypes.Zerg_Larva) {
				...
				if (bwapi.getSelf().getMinerals() >= 50 && startDrones<=WAVE1+1) {
					unit.morph(UnitTypes.Zerg_Drone);
					startDrones++;
					System.out.println("Drone Spawned. Current Drone count: "+startDrones);
				}
			}
		}
		...
		// collect minerals
		for (Unit unit : bwapi.getMyUnits()) {
			if (unit.getType() == UnitTypes.Zerg_Drone&& unit.isIdle()&&!unit.swarming) {
		...
		*/
		...
		for (Unit unit: bwapi.getMyUnits()) {
			if (unit.swarming&& unit.isIdle()) {
				//for (Unit enemy : bwapi.getEnemyUnits()){
					unit.attack(target.getPosition(), false);
					unit.swarming=true;
					if (unit.isSwarming()){
						System.out.println("Swarming1");
					}
					break;
				//}
		...
		// send all but one drone to attack
		for (Unit unit: bwapi.getMyUnits()) {
			if (unit.getType() == UnitTypes.Zerg_Drone && startDrones>=6 && sentDrones<WAVE1-1&&!unit.swarming) {
				System.out.println("Drone "+ sentDrones+ " Sent to Attack");
				sentDrones++;
				//for (Unit enemy : bwapi.getEnemyUnits()){
					unit.attack(target.getPosition(), false);
					unit.swarming=true;
					if (unit.isSwarming()){
						System.out.println("Swarming2"+" Sent Drones: "+sentDrones);
					}
					break;
				//}
			}
